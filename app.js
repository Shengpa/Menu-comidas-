...
async function init() {
  ...
  try {
    ...
    } else if (state.today.getDay() === 0 && state.apiKey) {
...
async function onGenerateClick() {
  if (!state.apiKey) { state.showSettings = true; render(); return; }
...
function render() {
  ...
  if (state.loading) {
    ...
    if (!state.current) {
      ...
      if (!esDomingo) html += `<div class="hint">Todavía no hay menú generado para esta semana. Se genera solo los domingos; si querés, generalo ahora a mano.</div>`;
    ...
    html += `
      <div class="modal-backdrop no-print" id="modal-backdrop">
        <div class="modal">
          ...
          <p class="hint">API key de Anthropic. Se guarda solo en este navegador (no se sube a ninguna base de datos).</p>
          <input type="password" id="api-key-input" placeholder="sk-ant-..." value="${escapeHtml(state.apiKey)}" />
          <button class="btn-secondary small" id="save-key-btn">Guardar key</button>
