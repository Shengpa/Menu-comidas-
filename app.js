...
Devolvé exactamente este formato JSON (sin explicaciones fuera del JSON):
{
  ...
  "comodin": {"dish": "plato de emergencia rapidísimo, menos de 15 minutos", "note": "por qué sirve para un día caótico"},
  "tip": "un consejo de cocina corto y útil para la semana"
}
Cada receta: 6 a 9 ingredientes, y 4 a 6 pasos cortos (una oración cada uno, sin números adentro del texto).`;
  return { system, user };
}
...
async function fetchDishImage(query) {
  ...
  return (await tryQuery(`${query} food`)) || (await tryQuery(query));
}

async function attachImages(weekData) {
  if (!state.pexelsKey) return weekData;
  await Promise.all((weekData.days || []).map(async (d) => {
    d.imageUrl = await fetchDishImage(d.dish);
  }));
  if (weekData.comodin) {
    weekData.comodin.imageUrl = await fetchDishImage(weekData.comodin.dish);
  }
  return weekData;
}
...
async function regenerateDay(dayName) {
  ...
  try {
    ...
    newDay.imageUrl = await fetchDishImage(newDay.dish);
  ...
  try {
    ...
    if (existing) {
      ...
      try {
        ...
        console.error(e);
        state.error = "No se pudo generar el menú automáticamente. Probá con el botón manual.";
      }
...
async function onGenerateClick() {
  ...
  try {
    ...
    console.error(e);
    state.error = "No se pudo generar el menú. Probá de nuevo en un momento.";
  }
  state.generating = false;
  render();
}
