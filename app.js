...
function formatStep(step) {
  // Compatibilidad con menús viejos donde "steps" eran strings simples, sin tiempo.
  if (typeof step === "string") return { text: step, minutes: null };
  return { text: step.text || "", minutes: Number.isFinite(step.minutes) ? step.minutes : null };
}
...
function stepsFormatSpec() {
  return `"steps": [{"text": "Picar la cebolla y el ajo en cubos chicos", "minutes": 5}, {"text": "Calentar el aceite en una sartén a fuego medio", "minutes": 2}, ...] — "minutes" es el tiempo estimado de ESE paso puntual (activo o de cocción/espera), un número entero. La suma aproximada de todos los "minutes" tiene que dar el "prep_minutes" total de la receta`;
}
        ...
        if (isExpanded) {
          ...
          (w.days || []).forEach((d) => {
            html += `<div class="hist-detail">
              ...
              <ol>${(d.steps || []).map((s) => { const fs = formatStep(s); return `<li>${escapeHtml(fs.text)}${fs.minutes != null ? ` <span class="step-time">· ${fs.minutes} min</span>` : ""}</li>`; }).join("")}</ol>
            </div>`;
          });
          if (w.comodin) {
            html += `<div class="hist-detail">
              ...
              <ol>${(w.comodin.steps || []).map((s) => { const fs = formatStep(s); return `<li>${escapeHtml(fs.text)}</li>`; }).join("")}</ol>
            </div>`;
          }
        }
        html += `</div>`;
    ...
    if (!state.current) {
      ...
      if (state.current.comodin) {
        ...
        html += `<div class="comodin">
          ...
          ${cSteps.length ? `
            <div class="section-label">Paso a paso</div>
            <ol>${cSteps.map((s) => { const fs = formatStep(s); return `<li>${escapeHtml(fs.text)}${fs.minutes != null ? ` <span class="step-time">· ${fs.minutes} min</span>` : ""}</li>`; }).join("")}</ol>
          ` : ""}
        </div>`;
      }
