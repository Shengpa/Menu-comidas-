...
function toLocalISODate(d) {
  // Ojo: NUNCA usar toISOString() acá — convierte a UTC y de noche (Argentina es UTC-3)
  // puede "saltar" al día siguiente del calendario. Esto arma la fecha con los
  // componentes locales (año, mes, día) tal como los ve el usuario en su reloj.
...
function scaleQty(qty, unit, servings = state.servings) {
  ...
  else if (unit === "cda" || unit === "cdita" || unit === "taza") val = Math.round(val * 2) / 2;
  else val = Math.max(1, Math.round(val)); // unidades enteras (huevos, cebollas, dientes de ajo...): sin fracciones
  return Number.isInteger(val) ? val : val.toFixed(1);
}

function formatIngredient(ing, servings = state.servings) {
  const val = scaleQty(ing.qty, ing.unit, servings);
  return ing.unit ? `${val} ${ing.unit} de ${ing.name}` : `${val} ${ing.name}`;
}

function formatStep(step) {
  // Compatibilidad con menús viejos donde "steps" eran strings simples, sin tiempo ni tipo.
  if (typeof step === "string") return { text: step, minutes: null, type: null, critical: false };
  return {
    text: step.text || "",
    minutes: Number.isFinite(step.minutes) ? step.minutes : null,
    type: step.type === "activo" || step.type === "pasivo" ? step.type : null,
    critical: step.critical === true,
  };
}
function renderStepLi(s) {
  const fs = formatStep(s);
  const icon = fs.type === "pasivo" ? "⏳" : fs.type === "activo" ? "🔪" : null;
  const timeLabel = fs.minutes != null
    ? ` <span class="step-time ${fs.type ? "step-time-" + fs.type : ""}">${icon ? icon + " " : "· "}${fs.minutes} min</span>`
    : "";
  const critTag = fs.critical ? `<span class="step-critical-tag">💡 clave</span> ` : "";
  return `<li class="${fs.critical ? "step-critical" : ""}">${critTag}${escapeHtml(fs.text)}${timeLabel}</li>`;
}
...
function stepsFormatSpec() {
  return `"steps": [{"text": "Picar la cebolla y el ajo en cubos chicos", "minutes": 5, "type": "activo", "critical": false}, {"text": "Dejar reposar 5 minutos antes de cortar", "minutes": 5, "type": "pasivo", "critical": true}, ...] — "minutes" es el tiempo estimado de ESE paso puntual, un número entero; la suma aproximada tiene que dar el "prep_minutes" total. "type" es "activo" (hay que estar haciendo algo con las manos: cortar, mezclar, revolver) o "pasivo" (tiempo de espera sin intervención: hervir, hornear, reposar, marinar) — cuando un paso sea "pasivo" y dure varios minutos, aprovechalo: el texto del paso siguiente puede indicar una tarea activa para hacer EN PARALELO mientras tanto (ej. "Mientras hierven las papas, picar la cebolla y el ajo"), en vez de dejar ese tiempo como espera muerta. "critical": true SOLO en 1 o 2 pasos por receta como máximo — los que de verdad determinan si el plato sale bien o mal (ej. el tiempo de reposo post-horneado, templar la leche antes de agregarla, el punto exacto de un hervor); el resto de los pasos van con "critical": false.`;
}

const SEQUENCE_RULES = `Ordená los pasos en la secuencia REAL en la que se cocina, sin saltos ni cosas fuera de orden: primero precalentar el horno si hace falta (apenas arranca la receta, no a mitad de camino), después toda la preparación en frío (cortar, pelar, mezclar, marinar), y recién ahí los pasos de cocción en el orden correcto de la técnica (por ejemplo: primero calentar la sartén/olla, después agregar el aceite, después el ingrediente — nunca "agregar a la sartén" antes de haber puesto la sartén al fuego). Si un paso requiere que algo ya esté listo de un paso anterior, ese paso anterior tiene que estar antes. Cada paso describe UNA acción concreta con su punto de cocción (ej: "hasta que dore", "hasta que hierva", "unos 5 minutos") para que alguien que nunca cocinó ese plato pueda seguirlo sin dudas.`;

const QUALITY_RULES = `Prestá atención al nivel de detalle de un cocinero profesional, no solo al orden: (1) Si la receta lleva vegetales que sueltan agua usados crudos dentro de un relleno, guiso o tarta (espinaca, acelga, zapallito, berenjena, champiñones), indicá cómo manejar esa humedad — precocinarlos y escurrirlos/exprimirlos bien antes de mezclarlos, o ajustar la cocción (por ejemplo hornear destapado) para que no quede acuoso; elegí la opción más simple para casa. (2) En salsas de tomate, si tiene sentido, corregí la acidez con una pizca de azúcar. (3) Sazoná por capas, no solo al final: agregá especias con lógica (nuez moscada en rellenos con lácteos/ricota, hierbas secas incorporadas a mitad de cocción de una salsa para que liberen aroma sin quemarse ni amargar, no todo crudo al final). (4) En platos horneados con salsa y relleno (tartas, canelones, lasañas, gratinados, pastel de papa), poné una capa fina de salsa o materia grasa en el fondo de la fuente ANTES de acomodar el relleno, para que no se pegue. (5) Especificá si el horno va tapado o destapado cuando eso cambia el resultado (por ejemplo, destapado para que se evapore humedad y gratine). (6) Tené en cuenta cuánto cambian de peso y volumen los ingredientes al cocinarse — las verduras de hoja (espinaca, acelga, rúcula) pierden gran parte de su volumen y sueltan líquido al cocinarlas, quedando en una fracción de la cantidad cruda: calculá la cantidad cruda necesaria para que, ya cocida y escurrida, alcance de verdad para el relleno o la porción como se describe en la receta, en vez de dejarla corta. (7) En preparaciones con carne picada moldeada (albóndigas, hamburguesas, buñuelos de carne), usá panada en vez de pan rallado seco — miga de pan remojada en leche o caldo — para que no queden secas ni compactas al cocinarlas, y sazoná la mezcla con algo más que sal y pimienta (orégano, pimentón, queso rallado) para que no quede insípida. (8) Toda salsa a base de tomate (triturado, puré, o salsa fileto) lleva al menos una hierba aromática: laurel durante la cocción, y albahaca u orégano fresco/seco al apagar el fuego. (9) Si en la receta se dora carne o verduras en una olla o sartén antes de armar una salsa en el mismo recipiente, aprovechá los restos dorados del fondo — agregá un chorro de vino, caldo o el líquido de la salsa y desglasá raspando el fondo con cuchara de madera antes de seguir, en vez de ignorar ese sabor. (10) Con pasta y salsa: nunca la sirvas escurrida en el plato con la salsa volcada encima. Retirá la pasta un minuto antes del punto, terminala salteada dentro de la sartén/cacerola junto con la salsa durante un minuto, agregando un cucharón del agua de cocción de la pasta para que la salsa se adhiera bien (mantecar). (11) Consistencia obligatoria entre ingredientes y pasos: todo condimento o ingrediente que mencionés en un paso (ej. "ajustar con nuez moscada") TIENE que figurar en la lista de "ingredients" con su cantidad — nunca nombres algo en un paso que no esté en la lista. Antes de responder, revisá que no falte ninguno. (12) En purés (de papa, batata, calabaza), usá una proporción real de materia grasa y líquido, no solo líquido — aproximadamente cada 1 kg de papas/batatas lleva unos 50 g de manteca y 100 ml de leche (ajustá proporcionalmente); un puré sin materia grasa o con muy poco líquido queda seco y gomoso. (13) Al cocinar carne picada u otra carne para un relleno o salsa junto con cebolla u otros vegetales, dorá/sellá la carne SOLA primero a fuego fuerte hasta que tome color, y recién después sumá la cebolla — si los cocinás todos juntos desde el arranque, la carne libera líquido y hierve en vez de dorarse. Una vez que agregaste la cebolla sobre la carne ya sellada, condimentá en ese momento (sal, pimienta, y lo que corresponda: pimentón, orégano, etc.) y dejá reducir bien el líquido que suelta la cebolla antes de usar esa mezcla como relleno, para que no quede empapado. (14) En platos clásicos argentinos con una versión tradicional bien conocida (pastel de papa, tarta pascualina, etc.), incluí los componentes clásicos esperados cuando no compliquen demasiado la receta casera (ej. huevo duro y aceitunas en el relleno del pastel de papa). (15) En pasteles de molde donde el puré va arriba Y abajo (pastel de papa, de batata, de zapallo), poné también una capa fina de puré en la base de la fuente antes de la carne, no solo por encima — así no se pega y las porciones salen más prolijas. (16) Timing de sazón sensible al calor: pimentón, comino y especias que se queman fácil se incorporan con el fuego apagado o bien bajo, nunca a fuego fuerte, para que no amarguen. (17) En salteados de vegetales, agregá una pizca de sal desde el arranque para ayudar a que suden y caramelicen mejor. (18) En purés de papa/batata: hervilas siempre partiendo de agua FRÍA con sal (nunca en agua ya hirviendo), y agregá la manteca y la leche TIBIAS (no frías de la heladera) apenas escurridas, mientras la papa sigue caliente. (19) En pasteles de molde: marcá la superficie del puré de cobertura con un tenedor antes de hornear para que gratine mejor, y agregá un paso final de reposo de 5 a 10 minutos después de sacarlo del horno, antes de servir, para que se asiente. Como referencia de proporción para una fuente de 4 a 6 porciones: unos 750 g de carne por 1,2-1,3 kg de vegetal de cobertura (papa, batata o zapallo).`;

const VARIETY_RULES = `Variedad real de principio a fin: además de no repetir la misma proteína ni la misma técnica dos días seguidos, tampoco repitas el mismo acompañamiento principal en más de un día de la semana (por ejemplo, si el lunes servís "pollo con arroz", el arroz como acompañamiento no puede volver a aparecer como base de otro día, aunque cambie la proteína — variá entre arroz, puré, papas, ensalada, vegetales salteados, polenta, etc. a lo largo de la semana). Incluí pescado o algún fruto de mar en al menos un día de la semana, salvo que las restricciones del usuario digan lo contrario (vegetariano, vegano, no come pescado, etc.).`;
...
function buildWeekPrompt(previousDishes, favoriteDishes, weekLabel) {
  const avoid = previousDishes.length
    ? `Platos que YA se hicieron en semanas anteriores — esto es una lista de exclusión ESTRICTA, no una simple referencia: antes de responder, revisá cada uno de los 5 platos nuevos Y el comodín contra esta lista y confirmá que ninguno es igual ni una variación menor (mismo plato con otro nombre, o la misma base con un cambio cosmético) de algo que ya salió: ${previousDishes.join(", ")}. El comodín NO es una excepción: tiene que variar semana a semana tanto como los otros platos — no caigas siempre en la misma idea (ej. omelette); rotá entre opciones bien distintas: tortilla de papas, quesadillas, fideos con manteca y queso, sandwich de milanesa, revuelto de vegetales, arroz con huevo frito, wrap, ensalada con atún o huevo, etc.`
  ...
  const favLine = favoriteDishes.length
    ...
    : "";
  const system = `Sos un cocinero profesional que escribe recetas caseras para una familia en Argentina, con la precisión de alguien que las va a testear él mismo antes de publicarlas. Respondé ÚNICAMENTE con un objeto JSON válido, sin texto antes ni después, sin markdown, sin comentarios. Las recetas son para 4 porciones exactas, sin restricciones alimentarias salvo que se indique lo contrario. Priorizá variedad real: en los 5 días de la semana, usá proteínas principales distintas en la mayor cantidad de días posible — ninguna proteína (pollo, carne vacuna, cerdo, pescado o mariscos, legumbres) puede aparecer en más de 2 de los 5 días, y nunca en días consecutivos; tampoco repitas la misma técnica de cocción dos días seguidos. Mezclá clásicos caseros argentinos (milanesas, tartas, guisos, pastas caseras, salteados, platos al horno) con comida internacional simple y fácil de conseguir en un supermercado argentino común (wok, curry suave, tacos, pasta italiana, comida asiática fácil). ${SEQUENCE_RULES} ${QUALITY_RULES} ${VARIETY_RULES} ${restrictionsLine()}`;
...
{
  "days": [
    {"day": "Lunes", "category": "2-3 palabras que describan el plato", "dish": "nombre del plato", "prep_minutes": 40, ${ingredientFormatSpec()}, ${stepsFormatSpec()}},
    ... (uno por cada día, Lunes a Viernes)
  ],
  "comodin": {"dish": "plato de emergencia rapidísimo, menos de 15 minutos", "note": "por qué sirve para un día caótico", ${ingredientFormatSpec()}, ${stepsFormatSpec()}},
  "tip": "un consejo de cocina corto y útil para la semana"
}
Cada receta de los 5 días: 6 a 9 ingredientes, y 4 a 7 pasos (revisá antes de responder que el orden sea el real, paso por paso, sin saltos). El comodín: 3 a 5 ingredientes que probablemente ya haya en casa, y 2 a 4 pasos.`;
  return { system, user };
}
...
function buildSingleDayPrompt(day, weekLabel, otherDishes, previousDishes, favoriteDishes) {
  ...
  const system = `Sos un cocinero profesional que escribe recetas caseras para una familia en Argentina, con la precisión de alguien que las va a testear él mismo antes de publicarlas. Respondé ÚNICAMENTE con un objeto JSON válido, sin texto antes ni después, sin markdown. La receta es para 4 porciones exactas. ${SEQUENCE_RULES} ${QUALITY_RULES} ${VARIETY_RULES} ${restrictionsLine()}`;
...
{"category": "2-3 palabras", "dish": "nombre del plato", "prep_minutes": 40, ${ingredientFormatSpec()}, ${stepsFormatSpec()}}
6 a 9 ingredientes, 4 a 7 pasos (revisá que el orden sea el real, sin saltos).`;
  return { system, user };
}
...
function pollinationsImageUrl(dish) {
  const prompt = `${dish}, warm flat-design food illustration, digital vector art style, appetizing warm colors, simple clean composition, no text, no letters`;
...
const DISH_TYPE_EN = [
  ...
  ["fajitas", "fajitas"], ["horno", "roasted oven dish"], ["albóndigas", "meatballs in tomato sauce"],
  ["estofado", "beef stew"], ["matambre", "rolled stuffed flank steak"], ["locro", "corn and bean stew"],
  ["carbonada", "beef vegetable stew"], ["pastel de carne", "meatloaf"], ["escalope", "breaded cutlet"],
  ["chop suey", "chop suey noodles"], ["arrollado", "rolled meat"], ["budín", "savory loaf"],
];
...
async function loadRecentWeeks() {
  ...
  const all = snap.docs.map((doc) => doc.data()).reverse();
  // El historial es "lo anterior": si la semana actual ya está guardada, no la contamos acá,
  // así no se duplica con el contador de la semana en curso.
  return all.filter((w) => w.mondayISO !== state.range?.mondayISO);
}
...
async function deleteWeekDoc(mondayISO) {
  const batch = db.batch();
  batch.delete(db.collection("weeks").doc(mondayISO));
  batch.delete(db.collection("checklists").doc(mondayISO));
  await batch.commit();
}
async function deleteWeekDocs(mondayISOs) {
  const batch = db.batch();
  mondayISOs.forEach((iso) => {
    batch.delete(db.collection("weeks").doc(iso));
    batch.delete(db.collection("checklists").doc(iso));
  });
  await batch.commit();
}
...
async function regenerateDay(dayName) {
  ...
  try {
    ...
    console.error(e);
    state.error = `No se pudo cambiar el plato de ${dayName} (${e?.message || "error desconocido"}). Probá de nuevo.`;
  }
...
async function onDeleteHistoryWeek(mondayISO) {
  try {
    await deleteWeekDoc(mondayISO);
    state.recentWeeks = state.recentWeeks.filter((w) => w.mondayISO !== mondayISO);
  } catch (e) {
    console.error(e);
    state.error = "No se pudo borrar esa semana del historial.";
  }
  render();
}
...
async function onClearAllHistory() {
  ...
  try {
    await deleteWeekDocs(state.recentWeeks.map((w) => w.mondayISO));
    state.recentWeeks = [];
  } catch (e) {
    console.error(e);
    state.error = "No se pudo borrar el historial.";
  }
  render();
}
...
function render() {
  ...
  if (state.loading) {
    ...
    html += `<div class="tab-bar no-print">
      <button class="tab-btn ${state.activeTab === "menu" ? "active" : ""}" data-tab="menu">Menú de la semana</button>
      <button class="tab-btn ${state.activeTab === "historial" ? "active" : ""}" data-tab="historial">Historial${state.recentWeeks.length ? ` (${state.recentWeeks.length})` : ""}</button>
    </div>`;

    if (state.activeTab === "historial") {
      html += `<div class="card no-print">
        <div class="card-title-row">
          <div class="card-title">Semanas anteriores</div>
          ${state.recentWeeks.length ? (
            !state.confirmClearHistory
              ? `<button class="btn-secondary small" id="clear-history-btn">Borrar todo</button>`
              : `<span class="confirm-text">¿Borrar todo el historial?</span>
                 <button class="btn-primary small" id="confirm-clear-history-btn">Sí, borrar</button>
                 <button class="btn-secondary small" id="cancel-clear-history-btn">Cancelar</button>`
          ) : ""}
        </div>`;
      if (!state.recentWeeks.length) {
        html += `<div class="hint">Todavía no hay semanas anteriores guardadas.</div>`;
      }
      state.recentWeeks.slice().reverse().forEach((w) => {
        const dishes = (w.days || []).map((d) => d.dish).join(" · ");
        const isExpanded = state.expandedHistoryWeek === w.mondayISO;
        const hServings = state.historyServings[w.mondayISO] || state.servings;
        html += `<div class="hist-block">
          <div class="hist-row">
            <button class="hist-toggle" data-toggle-hist="${escapeHtml(w.mondayISO)}">
              <span class="mono">${escapeHtml(w.weekRangeLabel)}:</span>
              <span>${escapeHtml(dishes)}</span>
              <span class="hist-caret">${isExpanded ? "▲" : "▼"}</span>
            </button>
            <button class="hist-delete" data-delete-week="${escapeHtml(w.mondayISO)}" title="Borrar esta semana del historial" aria-label="Borrar semana del ${escapeHtml(w.weekRangeLabel)}">✕</button>
          </div>`;
        if (isExpanded) {
          html += `<div class="servings-row" style="margin: 6px 0 10px;">
            <span class="hint" style="width:auto;">Porciones para esta semana:</span>
            ${SERVING_OPTIONS.map((n) => `<button class="chip ${hServings === n ? "chip-active" : ""}" data-hist-servings="${w.mondayISO}::${n}">${n}</button>`).join("")}
          </div>`;
          (w.days || []).forEach((d) => {
            html += `<div class="hist-detail">
              <div class="hist-detail-title">${escapeHtml(d.day)} — ${escapeHtml(d.dish)}</div>
              <ul>${(d.ingredients || []).map((i) => `<li>${escapeHtml(formatIngredient(i, hServings))}</li>`).join("")}</ul>
              <ol>${(d.steps || []).map(renderStepLi).join("")}</ol>
            </div>`;
          });
          if (w.comodin) {
            html += `<div class="hist-detail">
              <div class="hist-detail-title">Comodín — ${escapeHtml(w.comodin.dish)}</div>
              <ul>${(w.comodin.ingredients || []).map((i) => `<li>${escapeHtml(formatIngredient(i, hServings))}</li>`).join("")}</ul>
              <ol>${(w.comodin.steps || []).map(renderStepLi).join("")}</ol>
            </div>`;
          }
        }
        html += `</div>`;
    ...
    if (!state.current) {
      ...
      if (state.current.comodin) {
        ...
        html += `<div class="comodin">
          ...
          ${cSteps.length ? `
            <div class="section-label">Paso a paso</div>
            <ol>${cSteps.map(renderStepLi).join("")}</ol>
          ` : ""}
        </div>`;
      }
...
function bindEvents() {
  ...
  on("clear-history-btn", () => { state.confirmClearHistory = true; render(); });
  on("confirm-clear-history-btn", onClearAllHistory);
  on("cancel-clear-history-btn", () => { state.confirmClearHistory = false; render(); });
  app.querySelectorAll("[data-tab]").forEach((btn) => {
    btn.onclick = () => { state.activeTab = btn.getAttribute("data-tab"); render(); };
  });
  ...
  app.querySelectorAll("[data-delete-week]").forEach((btn) => {
    btn.onclick = () => onDeleteHistoryWeek(btn.getAttribute("data-delete-week"));
  });
  app.querySelectorAll("[data-toggle-hist]").forEach((btn) => {
    btn.onclick = () => {
      const iso = btn.getAttribute("data-toggle-hist");
      state.expandedHistoryWeek = state.expandedHistoryWeek === iso ? null : iso;
      render();
    };
  });
  app.querySelectorAll("[data-hist-servings]").forEach((btn) => {
    btn.onclick = () => {
      const [iso, n] = btn.getAttribute("data-hist-servings").split("::");
      state.historyServings = { ...state.historyServings, [iso]: parseInt(n, 10) };
      render();
    };
  });
