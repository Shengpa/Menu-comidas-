...
/* ---------- Fotos de los platos (Pexels, opcional) ---------- */
// Pexels tiene mucho mejor cobertura de fotografía genérica en inglés que de nombres
// puntuales de platos argentinos, así que traducimos la proteína/técnica principal
// para buscar algo visualmente parecido en vez del nombre literal del plato.
const FOOD_KEYWORDS_EN = [
  ["milanesa", "schnitzel breaded cutlet"], ["pechuga de pollo", "grilled chicken breast"],
  ["pollo", "roast chicken"], ["bife", "beef steak"], ["carne picada", "ground beef"],
  ["carne", "beef"], ["cerdo", "pork"], ["costillita", "pork ribs"], ["chorizo", "sausage"],
  ["panceta", "bacon"], ["merluza", "white fish fillet"], ["salmón", "salmon fillet"],
  ["atún", "tuna"], ["camarón", "shrimp"], ["langostino", "shrimp"], ["cordero", "lamb"],
  ["pescado", "fish fillet"], ["tarta", "savory pie quiche"], ["guiso", "beef stew"],
  ["lentejas", "lentil stew"], ["garbanzos", "chickpea stew"], ["curry", "curry rice"],
  ["tacos", "tacos mexican food"], ["ensalada", "fresh salad"], ["risotto", "risotto"],
  ["fideos", "pasta"], ["pasta", "italian pasta"], ["arroz", "rice dish"], ["sopa", "soup"],
  ["empanadas", "empanadas"], ["pizza", "pizza"], ["wok", "stir fry noodles"],
  ["pollo pot pie", "chicken pot pie"], ["asado", "grilled meat barbecue"],
];

function guessImageQuery(dish, category) {
  const text = `${dish} ${category || ""}`.toLowerCase();
  for (const [es, en] of FOOD_KEYWORDS_EN) {
...
async function attachImages(weekData) {
  if (!state.pexelsKey) return weekData;
  state.lastImageError = null;
  let successes = 0;
  await Promise.all((weekData.days || []).map(async (d) => {
    d.imageUrl = await fetchDishImage(d.dish, d.category);
    if (d.imageUrl) successes++;
  }));
  if (weekData.comodin) {
    weekData.comodin.imageUrl = await fetchDishImage(weekData.comodin.dish, null);
    if (weekData.comodin.imageUrl) successes++;
  }
  if (successes === 0) {
    state.imageWarning = `No se pudieron cargar las fotos esta vez${state.lastImageError ? ` (${state.lastImageError})` : ""}. El menú se guardó igual, solo sin imágenes.`;
  } else {
    state.imageWarning = null;
  }
  return weekData;
}
    ...
    if (!state.current) {
      ...
      (state.current.days || []).forEach((d) => {
        ...
        if (open) {
          html += `<div class="day-body">
            <hr class="divider" />
            ${d.imageUrl ? `<img class="day-img" src="${escapeHtml(d.imageUrl)}" alt="${escapeHtml(d.dish)}" loading="lazy" />` : ""}
      ...
      if (state.current.comodin) {
        ...
        html += `<div class="comodin">
          ${state.current.comodin.imageUrl ? `<img class="day-img" src="${escapeHtml(state.current.comodin.imageUrl)}" alt="${escapeHtml(state.current.comodin.dish)}" loading="lazy" />` : ""}
    ...
    html += `
      <div class="modal-backdrop no-print" id="modal-backdrop">
        <div class="modal">
          ...
          <p class="hint">Fotos de los platos (opcional): key gratis de Pexels. Con esto cada receta se muestra con una foto real.</p>
